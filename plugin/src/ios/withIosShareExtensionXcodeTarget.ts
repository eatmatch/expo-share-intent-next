...
function getMainAppDevelopmentTeam(pbx: any): string | null {
  ...
  for (const key in configs) {
    ...
    if (developmentTeam) {
      console.log(
        `[expo-share-intent-next] Found DEVELOPMENT_TEAM='${developmentTeam}' from main app configuration.`,
      );
  ...
  console.warn(
    "[expo-share-intent-next] No DEVELOPMENT_TEAM found in main app build settings. Developer will need to manually add Dev Team.",
  );
  return null;
}
...
export const withShareExtensionXcodeTarget: ConfigPlugin<Parameters> = (
  ...
  return withXcodeProject(config, async (config) => {
    ...
    if (existingTarget) {
      console.log(
        `[expo-share-intent-next] ${extensionName} already exists in project. Skipping…`,
      );
      return config;
    }
    ...
    if (groupExists) {
      console.log(
        `[expo-share-intent-next] ${extensionName} group already exists in project. Skipping…`,
      );
      return config;
    }
    ...
    console.log(
      `[expo-share-intent-next] Successfully created ${extensionName} target with files`,
    );
